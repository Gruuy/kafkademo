...
public class WordCount {
    public static void main(String[] args){
        ...
        //先拆分  再计数
        source.flatMapValues(a->{
            //分割成list  Locale.getDefault()获取本地国家代码  分割
            List<String> ts = Arrays.asList(a.toLowerCase(Locale.getDefault()).split("\\W+"));
            //输出  返回
            System.out.println(ts);
            return ts;
        //通过什么分类（return）  注意这里被分割之后  key是一个null值  value才是我们的target
        }).groupBy((s1,s2)->{
//            System.out.println(s1);
//            System.out.println(s2 );
            return s2;
        })
        //Materialized是用来描述store的   String是key类型  Long是value类型  第三个是状态存储的类型
        .count(Materialized.<String, Long, KeyValueStore<Bytes, byte[]>> as("counts-store"))
        //把store变回流
        .toStream()
        //再弄到topic上面   并制定序列化的类型
        .to("t2", Produced.with(Serdes.String(),Serdes.Long() ));
